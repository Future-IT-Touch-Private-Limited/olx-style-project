...
public partial class user_my_sale : System.Web.UI.Page
{
    ...
    DataTable GetData()
    {
        ...
        string strcon = "select * from transactions where product_by=@product_by and status='Success' ORDER BY sr DESC";
        SqlCommand cmd = new SqlCommand(strcon, con);
        cmd.Parameters.AddWithValue("@product_by", Request.Cookies["unun"].Value.ToString());
    ...
    public void all_count()
    {
        try
        {
            ...
            string myScalarQuery01 = "select count(*) from transactions where product_by=@product_by and status='Success'";
            SqlCommand myCommand01 = new SqlCommand(myScalarQuery01, con01);
            myCommand01.Parameters.AddWithValue("@product_by", Request.Cookies["unun"].Value.ToString());
